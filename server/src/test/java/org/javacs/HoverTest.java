...
public class HoverTest {
  ...
  public void fieldIdentifier() {
    assertThat(
        symbolAt("/org/javacs/example/SymbolUnderCursor.java", 9, 27), containsString("field"));
  }
  ...
  public void methodReference() {
    assertThat(
        symbolAt("/org/javacs/example/SymbolUnderCursor.java", 14, 65), containsString("method"));
  }
  ...
  public void methodParameterReference() {
    assertThat(
        symbolAt("/org/javacs/example/SymbolUnderCursor.java", 10, 32),
        containsString("methodParameter"));
  }

  @Test
  public void localVariableReference() {
    assertThat(
        symbolAt("/org/javacs/example/SymbolUnderCursor.java", 10, 16),
        containsString("localVariable"));
  }

  @Test
  public void throwsList() {
    assertThat(
        symbolAt("/org/javacs/example/HoverThrows.java", 11, 11),
        containsString("throws java.io.IOException"));
  }
  ...
  // Re-using the language server makes these tests go a lot faster, but it will potentially produce
  // surprising output
  // if things go wrong
  ...
  private String symbolAt(String file, int line, int character) {
    var pos =
        new TextDocumentPositionParams(
            new TextDocumentIdentifier(FindResource.uri(file)),
