...
public class SearchTest {
  ...
  private static Set<String> searchWorkspace(String query, int limit) {
    return server.workspaceSymbols(new WorkspaceSymbolParams(query)).stream()
  ...
  private static Set<String> searchFile(URI uri) {
    return server.documentSymbol(new DocumentSymbolParams(new TextDocumentIdentifier(uri))).stream()
        .map(result -> result.name)
        .collect(Collectors.toSet());
  }
