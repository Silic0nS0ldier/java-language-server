...
public class CodeActionProvider {
  ...
  public List<CodeAction> codeActionForDiagnostics(CodeActionParams params) {
    LOG.info(
        String.format(
            "Check %d diagnostics for quick fixes...", params.context.diagnostics.size()));
    var started = Instant.now();
    var file = Paths.get(params.textDocument.uri);
    var actions = new ArrayList<CodeAction>();
  ...
  private ClassTree findClassTree(CompileTask task, Range range) {
    var position =
        task.root().getLineMap().getPosition(range.start.line + 1, range.start.character + 1);
  ...
  private CharSequence extractRange(CompileTask task, Range range) {
    ...
    var start =
        (int) task.root().getLineMap().getPosition(range.start.line + 1, range.start.character + 1);
    var end =
        (int) task.root().getLineMap().getPosition(range.end.line + 1, range.end.character + 1);
