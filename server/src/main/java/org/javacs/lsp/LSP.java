...
public class LSP {
  ...
  private static void notifyClient(OutputStream client, String method, Object params) {
    ...
    var jsonText = toJson(params);
    var messageText =
        String.format("{\"jsonrpc\":\"2.0\",\"method\":\"%s\",\"params\":%s}", method, jsonText);
    writeClient(client, messageText);
  }
      ...
      void peek(Message message) {
        if (message.method.equals("$/cancelRequest")) {
          ...
          if (removed)
            ...
            LOG.info(
                String.format(
                    "Cannot cancel request %d because it has already started", params.id));
        }
      }
          ...
          default:
            {
              ...
              if (r.id != null) {
                error(
                    ...
                    r.id,
                    new ResponseError(
                        ErrorCodes.MethodNotFound, "Method not implemented: " + r.method, null));
              }
            }
        }
      } catch (Throwable e) {
