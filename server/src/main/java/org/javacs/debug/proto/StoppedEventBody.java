...
public class StoppedEventBody {
   ...
   * 'description' attribute is missing (but it must not be translated). Values: 'step',
   * 'breakpoint', 'exception', 'pause', 'entry', 'goto', 'function breakpoint', 'data breakpoint',
   * etc.
   */
  ...
  /**
   * Additional information. E.g. if reason is 'exception', text contains the exception name. This
   * string is shown in the UI.
   */
  public String text;
   ...
   * their stacktraces. - If the attribute is missing or false, only the thread with the given
