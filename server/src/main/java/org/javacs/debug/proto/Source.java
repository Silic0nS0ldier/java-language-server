...
public class Source {
   ...
   * the source if no sourceReference is specified (or its value is 0).
   */
  public String path;

  /**
   * If sourceReference > 0 the contents of the source must be retrieved through the SourceRequest
   * (even if a path is specified). A sourceReference is only valid for a session, so it must not be
   * used to persist a source.
   */
  public Integer sourceReference;

  /**
   * An optional hint for how to present the source in the UI. A value of 'deemphasize' can be used
   * to indicate that the source is not available or that it is skipped on stepping. 'normal' |
   * 'emphasize' | 'deemphasize'.
   */
  public String presentationHint;

  /**
   * The (optional) origin of this source: possible values 'internal module', 'inlined content from
   * source map', etc.
   */
