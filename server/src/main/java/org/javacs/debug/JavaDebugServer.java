...
public class JavaDebugServer implements DebugServer {
  ...
  class ReceiveVmEvents implements Runnable {
    ...
    private void process(com.sun.jdi.event.Event event) {
      ...
      if (event instanceof ClassPrepareEvent) {
        ...
        LOG.info(
            "ClassPrepareRequest for class " + type.name() + " in source " + relativePath(type));
  ...
  private Breakpoint enableDisabledBreakpoint(Source source, BreakpointRequest b) {
    LOG.info(
        String.format("Enable disabled breakpoint %s:%d", source.path, b.location().lineNumber()));
    b.enable();
  ...
  private Breakpoint enableBreakpointImmediately(
      Source source, SourceBreakpoint b, ReferenceType type) {
  ...
  private boolean tryEnableBreakpointImmediately(
      Source source, SourceBreakpoint b, ReferenceType type) {
    ...
    try {
      ...
      LOG.info(
          String.format(
              "No locations in %s for breakpoint %s:%d", type.name(), source.path, b.line));
      return false;
    }
    for (var l : locations) {
