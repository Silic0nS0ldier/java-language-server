...
class JavaLanguageServer extends LanguageServer {
  ...
  private JavaCompilerService createCompiler() {
    Objects.requireNonNull(
        workspaceRoot, "Can't create compiler because workspaceRoot has not been initialized");
  ...
  public void didChangeWatchedFiles(DidChangeWatchedFilesParams params) {
    for (var c : params.changes) {
      ...
      if (name.endsWith(".bzl")
          ...
          || name == "WORKSPACE") {
        LOG.info(
            "Compiler needs to be re-created because Bazel build definition file '"
                + file
                + "' has changed.");
        modifiedBuild = true;
        return;
      ...
      if (name == "pom.xml") {
        LOG.info(
            "Compiler needs to be re-created because a Maven project file '"
                + file
                + "' has changed.");
        modifiedBuild = true;
        return;
