...
public class InMemoryJavaFileManagerTest {
    ...
    void listSourcePathReturnsInMemorySources() throws IOException {
        var inputs = List.<JavaFileObject>of(
            new StringSourceJavaFileObject(URI.create("mem:///com/example/Foo.java"), "package com.example; class Foo {}", 0),
            new StringSourceJavaFileObject(URI.create("mem:///com/example/Bar.java"), "package com.example; class Bar {}", 0),
            new StringSourceJavaFileObject(URI.create("mem:///com/other/Baz.java"), "package com.other; class Baz {}", 0)
        );
        var fm = InMemoryJavaFileManager.create(inputs);

        var result = new ArrayList<JavaFileObject>();
        for (var obj : fm.list(StandardLocation.SOURCE_PATH, "com.example", Set.of(JavaFileObject.Kind.SOURCE), false)) {
            result.add(obj);
        }
    ...
    void inferBinaryNameForInMemorySource() {
        ...
        assertEquals("com.example.Foo", fm.inferBinaryName(StandardLocation.SOURCE_PATH, inputs.get(0)));
    }
