...
final class InMemoryJavaFileManagerImpl extends ForwardingJavaFileManager<StandardJavaFileManager> implements JavaFileManager {
    ...
    private static @Nullable URI uriForFileObject(Location location, String packageName, String relativeName) {
        try {
        ...
        if (kinds.contains(JavaFileObject.Kind.SOURCE)
            ...
            if (recurse) {
                for (var entry : packageIndex.entries()) {
                    ...
                    if (pkg.equals(packageName) || pkg.startsWith(packageName + ".") || packageName.isEmpty()) {
