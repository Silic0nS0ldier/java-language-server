 ...
 *
 * <p>URIs should follow a path convention matching the Java package structure.
 * For in-memory sources, use the {@code mem} scheme (e.g., {@code mem:///com/example/Foo.java}).
 *
 * @param uri Unique identifier for this source file.
 * @param content The full source text.
 * @param version Monotonically increasing version number for change tracking.
 *                Use 0 for unversioned sources.
 */
public record SourceText(URI uri, String content, long version) {
    ...
    /**
     * Extracts the package name declared in this source.
     * Returns the empty string for the default (unnamed) package.
     */
    ...
    /**
     * Derives the simple class name from the URI's file name.
     * For example, {@code mem:///com/example/Foo.java} yields {@code "Foo"}.
     */
    ...
    /**
     * Derives the binary name (fully-qualified class name) from the package declaration
     * and URI file name. For example, a source with {@code package com.example;} at URI
     * {@code mem:///com/example/Foo.java} yields {@code "com.example.Foo"}.
     */
