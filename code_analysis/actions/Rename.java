 ...
 *
 * <p>Builds on {@link FindReferences} — the same discovery logic locates every
 * declaration and use, and each reference becomes a {@link TextEdit} that replaces the
 * identifier with the new name.
 *
 * <p>When renaming a class, constructor declarations sharing that name are renamed too.
 * Symmetrically, renaming a constructor also renames its enclosing class and any
 * sibling constructors, so the source remains valid Java.
 *
 * <h2>Example</h2>
 * <pre>{@code
...
public final class Rename {
     ...
     * @return one {@link TextEdit} per reference; empty if no symbol is present at the
     *         offset
     * @throws InvalidIdentifierException           if {@code newName} is not a valid
     *                                              Java identifier or is a reserved
     *                                              word
     * @throws FindReferences.NoSuchSourceException if {@code uri} is not present in the
     *                                              compilation
     * @throws IllegalStateException                if the compilation has no semantic
     *                                              model
     */
     ...
     * @throws InvalidIdentifierException if {@code newName} is not a valid Java
     *                                    identifier or is a reserved word
    ...
    /**
     * Class and constructor declarations share their name in source, so a rename of one
    ...
    private static void validateIdentifier(String newName) {
        ...
        if (!Character.isJavaIdentifierStart(newName.charAt(0))) {
            throw new InvalidIdentifierException(
                "Invalid Java identifier: '" + newName + "'");
        }
    ...
    /**
     * Thrown when a proposed rename identifier is not a valid Java identifier or is a
     * reserved word.
     */
