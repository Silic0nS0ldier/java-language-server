 ...
 *
 * <p>The result includes both the declaration (when present in the compiled sources) and all use
 * sites. References are located exactly on the identifier — not on the enclosing expression —
 * matching what an editor would highlight.
 *
 * <h2>Example</h2>
 *
 * <pre>{@code
...
public final class FindReferences {
  ...
  /**
   * Locate the symbol at the given character offset in the specified source and find all references
   * to it in the compilation.
   *
   ...
   * @return an immutable list of references, or an empty list if no symbol was found at that offset
   ...
   */
  public static ImmutableList<Reference> atPosition(Compilation compilation, URI uri, int offset) {
   ...
   *
   * <p>This is useful for actions that need the element itself (e.g. to expand a rename to related
   * declarations) rather than only its references.
   *
   * @param compilation the compilation to search
   ...
   * @throws IllegalStateException if the compilation has no {@link SemanticModel}
   */
  public static @Nullable Element resolveElement(Compilation compilation, URI uri, int offset) {
   ...
   *
   * <p>References are matched by element equality — for javac elements within a single compilation
   * this is reference equality on the underlying symbol.
   *
   * @param compilation the compilation to search
  ...
  /**
   * Visits every identifier-like node in a compilation unit, recording those whose resolved element
   * equals the target.
   */
  private static final class ReferenceScanner
    ...
    ReferenceScanner(
        Trees trees, SourcePositions positions, CompilationUnitTree root, Element target) {
      this.trees = trees;
    ...
    public Void visitMemberReference(
        MemberReferenceTree node, ImmutableList.Builder<Reference> results) {
      if (isTarget(getCurrentPath())) {
        addRange(node, node.getName(), Reference.Kind.USE, results);
      }
    ...
    /**
     * Find the offset of {@code name} within the source span {@code [start, end)}. Simple substring
     * scan — sufficient because the name is always somewhere in the span of any tree that
     * references it.
     */
