...
public class RenameTest {
  ...
  void renameField() {
    ...
    assertFalse(
        updated.contains("value"), "No remaining occurrences of the old name, got:\n" + updated);
  }
  ...
  void renameClassAlsoRenamesConstructors() {
    ...
    var updated = applyEdits(source, edits);
    assertTrue(updated.contains("class Bar"), "Class declaration should be renamed:\n" + updated);
    assertTrue(
        updated.contains("Bar() {}"), "Constructor declarations should be renamed:\n" + updated);
    assertTrue(
        updated.contains("Bar(int x) {}"),
        "Overloaded constructor declarations should be renamed:\n" + updated);
    assertTrue(
        updated.contains("static Bar make()"), "Return type usage should be renamed:\n" + updated);
    assertTrue(
        updated.contains("new Bar()"), "Constructor invocation should be renamed:\n" + updated);
    assertFalse(
        updated.contains("Foo"), "No remaining occurrences of the old name, got:\n" + updated);
  }
  ...
  void renameConstructorAlsoRenamesClass() {
    ...
    assertTrue(updated.contains("class Bar"), "Class declaration should be renamed:\n" + updated);
    assertTrue(
        updated.contains("Bar() {}"), "Constructor declaration should be renamed:\n" + updated);
  }
  ...
  void rejectsEmptyName() {
    var compilation =
        Compilation.create(List.of(SourceText.of("Foo.java", "class Foo { int x = 0; }")));
    var ex =
        assertThrows(
            Rename.InvalidIdentifierException.class,
  ...
  void rejectsInvalidIdentifierStartCharacter() {
    var compilation = Compilation.create(List.of(SourceText.of("Foo.java", "class Foo {}")));
    assertThrows(
        Rename.InvalidIdentifierException.class,
  ...
  /**
   * Apply a list of edits to a single source. Edits are sorted by descending start offset so
   * earlier edits do not shift the offsets of later ones.
   */
