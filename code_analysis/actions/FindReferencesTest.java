...
public class FindReferencesTest {
    ...
    void findTypeReferencesAcrossFiles() {
        var base = SourceText.of("com/example/Base.java", """
            ...
            public class Base {}
            """);
        var user1 = SourceText.of("com/example/UserA.java", """
            package com.example;
            public class UserA {
                Base b = new Base();
            }
            """);
        var user2 = SourceText.of("com/example/UserB.java", """
            package com.example;
        ...
        assertTrue(refs.size() >= 5, "Expected at least 5 references but got " + refs.size() + ": " + refs);

        // At least one reference in each user file
        assertTrue(refs.stream().anyMatch(r -> r.uri().equals(user1.uri())),
            "Expected reference in UserA");
        assertTrue(refs.stream().anyMatch(r -> r.uri().equals(user2.uri())),
            "Expected reference in UserB");
    }
    ...
    void referenceRangesPointAtIdentifier() {
        ...
        for (var ref : refs) {
            ...
            assertEquals(1, length,
                "Reference range should be exactly the identifier length: " + ref);
            assertEquals("x",
    ...
    void findConstructorReferences() {
        ...
        assertTrue(refs.size() >= 2,
            "Expected constructor declaration + use, got: " + refs);
    }
    ...
    void forElementDirect() {
        ...
        var xField = typeFoo.getEnclosedElements().stream()
    ...
    void lineAndColumnAreOneBased() {
        ...
        for (var ref : refs) {
            assertTrue(ref.range().startLine() >= 1,
                "Line should be 1-based, got: " + ref.range().startLine());
            assertTrue(ref.range().startColumn() >= 1,
