 ...
 *
 * <p>Create instances via the static factory methods {@link #create(Collection)} or
 * {@link #create(Collection, CompilerOptions)}.
 *
 * <p>Provides access to parsed syntax trees, diagnostics, and (when analysis succeeds)
 * a {@link SemanticModel} for type-level queries.
 *
 * <h2>Example</h2>
 * <pre>{@code
...
public final class Compilation {
    ...
    /**
     * Package-private factory for constructing a Compilation from pre-assembled parts.
     * Used by {@link IncrementalCompiler} which manages the javac lifecycle itself.
     */
    static Compilation fromParts(
        ...
        return new Compilation(syntaxTrees, diagnostics, phase, semanticModel, internalErrors, compilerOutput);
    }
    ...
    public static Compilation create(Collection<SourceText> sources, CompilerOptions options) {
        ...
        var fileObjects = sources.stream()
    ...
    /**
     * Parsed syntax trees keyed by source URI.
     * Empty if parsing failed entirely (check {@link #internalErrors()}).
     */
    ...
    /**
     * Semantic model providing access to type information, symbol resolution, and
     * tree-to-element mappings. Available when compilation at least attempted analysis
     * (even if analysis produced errors).
     *
    ...
    /**
     * Internal errors (exceptions) that occurred during compilation.
     * These are distinct from {@link #diagnostics()} — they represent failures in the
     * compilation process itself, not problems in the source code.
     */
