 ...
 * automatic release, the prior borrow's {@link Borrow#task task} must not be used — doing so
 * yields undefined behaviour from javac (typically {@code NullPointerException} deep inside
 * the compiler).
 *
 * <p>This class is not thread-safe. All method calls must be serialised by the caller.
 *
 * <p><b>This is an internal implementation detail. It relies on non-public javac APIs that
 * may change between JDK versions.</b>
 */
final class ReusableCompiler {
     ...
     *
     * <p>Any prior outstanding {@link Borrow} is automatically released. Its {@code task}
     * must not be used after this call — hold onto only the most recent Borrow.
     *
     * @return a fresh Borrow encapsulating the task. Close it when finished (or discard —
     *         the next call to {@code getTask} will release it).
     */
    Borrow getTask(
            Writer out,
        ...
        JavacTask task = JAVAC_TOOL.getTask(
            out, fileManager, diagnosticListener, options, classes, compilationUnits, currentContext);
    ...
    final class Borrow implements AutoCloseable {
        /**
         * The javac task for this borrow. Valid only while this Borrow is the compiler's
         * active borrow — i.e. until either {@link #close} is called on it, or a subsequent
         * {@link ReusableCompiler#getTask} call supersedes it.
         */
        ...
        /**
         * Release resources for this compilation and make the context available for reuse.
    ...
    /**
     * A javac {@link Context} that installs reusable variants of {@code JavaCompiler}
     * and {@code Log}, and supports clearing compilation-specific state between uses.
     */
    static final class ReusableContext extends Context implements TaskListener {
        ...
        /**
         * A Log that supports clearing state between compilations and lazily
         * re-acquires the diagnostic listener from the context.
         */
