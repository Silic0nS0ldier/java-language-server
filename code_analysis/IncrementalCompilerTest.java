...
public class IncrementalCompilerTest {
    ...
    void multipleIncrementalUpdates() {
        try (var compiler = IncrementalCompiler.create()) {
            ...
            // v1 — add a field
            compiler.addOrUpdateSource(new SourceText(
                URI.create("mem:///Foo.java"), "class Foo { int a; int b; }", 1));
    ...
    void crossReferenceAfterUpdate() {
        try (var compiler = IncrementalCompiler.create()) {
            compiler.addOrUpdateSource(SourceText.of("com/example/Base.java",
                "package com.example; public class Base { }"));
            compiler.addOrUpdateSource(SourceText.of("com/example/Child.java",
            ...
            compiler.addOrUpdateSource(new SourceText(
                ...
                "package com.example; public class Base { public int value = 1; }", 1));
    ...
    void setSourcesReplacesAll() {
        try (var compiler = IncrementalCompiler.create()) {
            ...
            compiler.setSources(List.of(
                SourceText.of("Baz.java", "class Baz {}")
            ));
            var result = compiler.compile();
    ...
    void setSourcesIdenticalIsNoOp() {
        try (var compiler = IncrementalCompiler.create()) {
            var sources = List.of(
                ...
                SourceText.of("Bar.java", "class Bar {}")
            );
    ...
    void semanticModelAvailableAfterRecompile() {
        try (var compiler = IncrementalCompiler.create()) {
            ...
            compiler.addOrUpdateSource(new SourceText(
                URI.create("mem:///Foo.java"), "class Foo { String y; }", 1));
            var r2 = compiler.compile();
    ...
    void diagnosticsReportedOnEachCompile() {
        try (var compiler = IncrementalCompiler.create()) {
            ...
            // Fix the error
            compiler.addOrUpdateSource(new SourceText(
                URI.create("mem:///Bad.java"), "class Bad { int x = 42; }", 1));
            var r2 = compiler.compile();
            assertNoDiagnosticErrors(r2);
        }
    }
